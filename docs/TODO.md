...
- [ ] D2. Hook seam: a Hook trait next to OfflineStorage and OfflineRemote at the std client layer, driven by the D1 events, with filters (on pull, on push, on conflict). Implementations (notification, log, exec) live in Neverest, not here.
...
- [ ] Consumer feedback from cardamum-android to fold in (flagged in its docs/io-offline-migration.md): the WantsLookupObject dedup assumes a link id names immutable bytes, but contact replicas sharing a UID diverge legitimately (cardamum answers the lookup empty on purpose); and the m:n membership adapters (JMAP, Google) map Add-with-OfflineOrigin to a membership patch, a contract worth naming in the OfflineChange docs.
- [ ] Copy placeholder whose origin died: when the body is cached locally the create could degrade to an append instead of staying pending forever (the consumer-side fallback is documented on OfflineChange::Add; an engine-side degrade needs a signal that the rejection is permanent).
- [ ] Streaming bodies: OfflineFetchedItem carries Vec<u8>, fine for PIM, a blocker for large items; revisit only if files ever enter scope (they are out of scope per DELTA_PLAN).
