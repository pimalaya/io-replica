...
The deep design of io-replica: the model, the merge semantics and the operational details that outgrow the src/lib.rs header. The header is the architecture overview and the entry point; read it first, this document assumes it. Where a statement here conflicts with the code, the code wins; please flag it.

## Where io-replica fits

io-replica is an **offline-first replica engine library**: it maintains a local replica of remote collections of items (mail first, contacts and calendar next), usable fully offline, and reconciles that replica with the remote through a three-way merge against a stored base. Sync is a consequence of offline editing, not the primary goal. The full design lives in the [pimdir SPEC](https://github.com/pimalaya/pimdir/blob/master/SPEC.md).
...
2. **Std client** (`client` feature): a blocking driver, `ReplicaClient`, that services those `Wants` through a `ReplicaStorage` and a `ReplicaRemote` trait the consumer implements.
...
- An `ReplicaObject` (object.rs) is the content-addressed body: a `ReplicaHash` (a collision-resistant content hash) plus a byte size. Stored once, the bytes live out of band at blobdir/<hash>, refcounted so copy, move and undelete are reference edits. Where content is immutable the object never changes; where it is mutable an edit is a new object (new hash). References derive from placement pointers (`ReplicaPlacement::object` and `ReplicaBase::object`): the consumer maintains the counts by diffing each `UpsertPlacement` against the row it replaces and releasing both pointers of a `DropPlacement`, and may garbage-collect an object no placement points at. `StoreObject` stores bytes and takes no reference of its own.
- An `ReplicaPlacement` (placement.rs) is one item's presence in one collection. It pins a logical item to a single collection through the protocol `ReplicaHandle` (an IMAP UID, a WebDAV href, a JMAP id; always a string so non-integer ids are a non-issue), and carries the per-location mutable state.

Many placements may point at one object: this is the dedup and unified-view mechanism. The cross-collection identity is the `ReplicaLinkId` (placement.rs): a stable content id (the Message-ID header, a vCard or iCalendar UID), never derived from a per-copy value a provider may rewrite. A body present under one collection is linked into another by `ReplicaLinkId`, so a copied or shared message is fetched and stored once.

### The level ladder

Each placement sits at one rung of a strict ladder (`ReplicaLevel`), where each rung includes the one below:
...
- `Meta`: a minimal summary is cached (a list row, enough to resolve the `ReplicaLinkId`), held in the placement's `meta` field as an opaque blob.
...
Each yield is paired with the `ReplicaArg` the driver feeds back: `Enumerate(ReplicaRemoteSnapshot)`, `Fetch(Vec<ReplicaFetchedItem>)`, `Push(Vec<ReplicaPushResult>)`, `Load(ReplicaLoaded)`, `LookupObject(BTreeMap<ReplicaLinkId, ReplicaHash>)`, `Write`. Outbound effects travel as two payload types (change.rs): `ReplicaChange` is what to push to the remote (`Add`, `Remove`, `SetFlags`, `Update`), and `ReplicaWriteOp` is what to persist locally (`UpsertPlacement`, `DropPlacement`, `StoreObject`, `SetCheckpoint`). The consumer applies a `ReplicaWriteOp` batch atomically. An `ReplicaChange::Add` carries an optional `ReplicaOrigin` (the source collection and handle, for a server-side copy that avoids re-uploading the body), an optional object `ReplicaHash` (the stored body to upload for a genuine append), the `ReplicaFlags` to create the member with (an IMAP APPEND flag list; a copy may ignore it, the skew reconciles on the next sync), and an optional `ReplicaLinkId` (the idempotency key for a retried add); a `ReplicaChange::Remove` carries an optional `to` collection (a move, a server-side UID MOVE), or `None` for a plain delete the consumer routes to trash; a `ReplicaChange::Update` replaces a member's content in place, gated on the last-synced revision (`if_match`).
...
The remote payloads (remote.rs) are the seam's vocabulary: a `ReplicaRemoteItem` is one enumerate row (handle and flags, deliberately no `ReplicaLinkId` and no body, because an IMAP SEARCH returns just UIDs and the link id is resolved later at the `Meta` fetch); a `ReplicaRemoteSnapshot` carries the observed items, the `vanished` handles, a `complete` flag, and the new checkpoint; a `ReplicaFetchedItem` carries the resolved `ReplicaLinkId`, the `ReplicaMeta`, and the body at `ReplicaTier::Full`; a `ReplicaPushResult` carries the per-change `ReplicaPushOutcome` (`Accepted` or `Rejected`), plus an `assigned` handle the engine rekeys a confirmed create to.
...
`sync` loads local state, enumerates the remote (full or delta), then runs a three-way merge of Local, ReplicaBase and ReplicaRemote per placement, keyed on the handle. The merge compares per-placement identities (the flag set, and for mutable-content backends the content `revision`), never raw bytes.
...
## ReplicaCheckpoint and sync depths

The checkpoint is an opaque token the engine round-trips. An incremental sync passes the stored checkpoint as the enumerate cursor; the consumer turns that into a QRESYNC `CHANGEDSINCE` plus a new-UID search and returns a delta snapshot (`complete = false`) with explicit `vanished` handles. A full sync (`ReplicaSyncOptions.full`) ignores the stored checkpoint, so the enumerate is asked for the whole remote and the merge reconciles the complete spine: it re-adds any locally-missing message and drops any local phantom. This is the recovery path for a replica that has drifted out of sync.

`ReplicaSyncOptions.push` is the second depth: when false the source is treated read-only, local changes are kept dirty and never pushed (permission gating), while remote-won changes are still pulled.

In the Android app these depths surface as three user actions: Refresh (an incremental sync), Resync (a full sync, `full = true`), and Download-all (an `upgrade` of a collection's spine to `ReplicaTier::Full`). The checkpoint itself, on IMAP, is a serialized sync state of UIDVALIDITY plus HIGHESTMODSEQ plus the highest UID.

## Driving the engine from an app

The std client (client.rs) is the reference driver: `ReplicaClient<S, R>` wraps a consumer `ReplicaStorage` and a consumer `ReplicaRemote`, and its generic `run` loop services each yield by calling the matching trait method and resuming with the reply. The five verbs are exposed as `open`, `upgrade`, `mutate`, `sync` and `rekey`. A desktop or Neverest consumer backs `ReplicaRemote` with io-email's blocking clients and `ReplicaStorage` with sqlite plus a blob dir.

A consumer may also drive the coroutines directly without `ReplicaClient`: the Himalaya Android app does, servicing each yield over two Kotlin transports reached by JNI (io-imap as the remote seam, a SQLite index plus a blob dir as the storage seam). That wiring, the JSON contract over JNI, and the IMAP enumerate are app concerns and live in that repo; see the himalaya-android-m3 ARCHITECTURE. Two engine-level facts matter here: a `ReplicaHandle` is any string (so a consumer's storage keys stay protocol-neutral), and a consumer that does not yet build creates reports `ReplicaChange::Add` as `Rejected` so the engine keeps the placeholder rather than dropping a member.
...
- `mutate`'s `Copy` stages a `ReplicaStatus::Created` placement in the target collection under a provisional placeholder handle, carrying its `ReplicaOrigin` (the source `ReplicaCollectionId` plus `ReplicaHandle`), and leaves the source untouched.
- `sync`'s merge turns that `Created` placement into a `ReplicaChange::Add { handle, link_id, flags, origin, object }`: `origin` set so the push reuses a server-side copy (no body re-upload), `object` carrying the stored body's hash for an append, `flags` the flag set to create with, `link_id` as the retry idempotency key. The placeholder is stashed in `pending_creates`.
...
- `mutate`'s `Move` tombstones the source carrying its destination in `origin` (no target placement), so the merge pushes one `ReplicaChange::Remove { handle, to: Some(target) }`: the consumer issues a single atomic server-side UID MOVE, never a copy-then-delete with a window where the message is on neither side. The target picks it up on its own next enumerate. (A plain delete is the same `Remove` with `to = None`, routed to trash.)
...
src/
  ...
  coroutine.rs    ReplicaCoroutine / ReplicaCoroutineState / ReplicaYield / ReplicaArg
  object.rs       ReplicaObject, ReplicaHash (content-addressed body)
  placement.rs    ReplicaPlacement, ReplicaHandle, ReplicaLinkId, ReplicaMeta, ReplicaFlags, ReplicaLevel, ReplicaStatus, ReplicaBase, ReplicaOrigin
  collection.rs   ReplicaCollection, ReplicaCollectionId, ReplicaCheckpoint
  change.rs       ReplicaChange (push) + ReplicaWriteOp (persist)
  remote.rs       ReplicaRemoteItem, ReplicaRemoteSnapshot, ReplicaFetchedItem, ReplicaPushResult, ReplicaTier
  storage.rs      ReplicaLoaded (the load reply)
  open.rs         ReplicaOpen          load-only, fully offline
  upgrade.rs      ReplicaUpgrade       pull a level, dedup before fetch
  mutate.rs       ReplicaMutate        local flag/remove, mark dirty, no network
  sync.rs         ReplicaSync          enumerate + three-way merge + push + checkpoint
  rekey.rs        ReplicaRekey         rebuild after a handle-space change, carry by link id
  client.rs       (client) ReplicaClient: ReplicaStorage + ReplicaRemote traits, blocking run loop
```
...
One thing is defined ahead of its first use, so do not mistake it for dead code: the `ReplicaCollection` struct (with its `enumerated` flag) is defined but no verb produces or consumes it (`ReplicaLoaded` carries placements plus checkpoint directly); treat it as a reserved carrier for the collection list. A handle-space change (an IMAP UIDVALIDITY bump renumbers every handle) is `rekey`'s job: a consumer that detects the change runs it instead of a full sync, which would drop the whole spine and every pending change with it. The residual limitation is that rekey matches by link id, so pending state on placements that never resolved one (a probed-only spine) is dropped, and said so in the report.
