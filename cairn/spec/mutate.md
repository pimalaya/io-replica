...
- `Edit` — store a new body and repoint the placement at it (full level, dirty),
  ...
  conflict time.
- `Copy` — stage a `Created` placement in a target carrying the source origin, so
  the push is a server-side copy (no body re-upload); the source is untouched.
- `Move` — tombstone the source carrying its destination origin, so the push is
  one atomic server-side move.
- `Add` — see below.
