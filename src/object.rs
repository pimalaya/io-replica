...
//!
//! An object is one of the two identity axes (see [`crate::placement`]
//! for the other). Where an item's content is immutable the object never
//! changes; where it is mutable an edit is a new object and the old one
//! is dereferenced. Many placements may point at one object, which is the
//! dedup and unified-view mechanism.

crate::replica_id! {
    /// The byte-exact identity of an object, naming it in the object store.
    ///
    /// A collision-resistant content hash (truncated SHA-256, say). Size
    /// is a cheap pre-check only: identical bytes have identical size,
    /// but equal size does not imply equal bytes.
